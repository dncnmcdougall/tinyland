...
class Renderer(renderer.Renderer):
    ...
    def setup(self):
        ...
        cv2.setWindowProperty(Renderer.WINDOW_TITLE, cv2.WND_PROP_AUTOSIZE, cv2.WINDOW_NORMAL)

    def toggle_fullscreen(self):
        cur = cv2.getWindowProperty(Renderer.WINDOW_TITLE, cv2.WND_PROP_FULLSCREEN)
        if cur == cv2.WINDOW_NORMAL:
            cv2.setWindowProperty(Renderer.WINDOW_TITLE, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
        else:
            cv2.setWindowProperty(Renderer.WINDOW_TITLE, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_NORMAL)
    ...
    def show_calibration_markers(self):
        ...
        marker = cv2.rotate(marker, cv2.ROTATE_90_CLOCKWISE)
        image[self.height - M_SZ : self.height, self.width - M_SZ : self.width] = marker
        marker = cv2.rotate(marker, cv2.ROTATE_90_CLOCKWISE)
        image[self.height - M_SZ : self.height, 0:M_SZ] = marker

        self._display_frame(image)

    def render(self, ctx, image=None):
        ...
        for shape in ctx.shapes:
            if isinstance(shape, context.Rectangle):
                ...
                cv2.fillPoly(image, pts=[transformed_corners], color=shape.color)
            elif isinstance(shape, context.Circle):
                center = (int(shape.center.x), int(shape.center.y))
                image = cv2.circle(image, center, int(shape.radius), color=shape.color, thickness=-1)
                ...
                for c in range(0, 3):
                    try:
                        image[y1:y2, x1:x2, c] = alpha_s * file_image[:, :, c] + alpha_l * image[y1:y2, x1:x2, c]
                    except ValueError:
                        image = image_save
